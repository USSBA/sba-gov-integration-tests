describe("Local Assistance Find", function(){
    ...
    it("can search for offices by office type", function(){
        ...
        cy.visit("/local-assistance/find")
        cy.get("input#zip").type("20024")
        cy.get("[data-cy='officetype-select']").find(".Select-arrow-zone").click()
        cy.get("[data-cy='officetype-select']").contains("Certified Development Company").click();
        cy.get("[data-cy='search button']").click()
        cy.wait("@OfficeSearch")
        expect(cy.get("#office-results").as("Results")).to.exist
        // Claims it has 5 results
    ...
    it("displays office info in search results", function(){
        cy.visit("local-assistance/find?type=SCORE%20Business%20Mentoring&address=21146&pageNumber=1")
    ...
    it("displays all office details when they exist", () => {
        ...
        cy.get("[data-cy='contact address']").first().should("have.text", "4091 3rd St SW, Suite 5300")
        cy.get("[data-cy='contact phone']").first().should("have.text", "(202) 205-6780")
