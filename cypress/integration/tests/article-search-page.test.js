describe("Article Search Page", function () {
    beforeEach(function () {
    ...
    it("displays office results when an office is selected in the office dropdown", function(){
        const expectedOfficeOption = this.SBAOffices[1]
        cy.server()
        ...
        cy.get("[data-cy='office']").as("OfficeDropdown").within((dropdown) => {
            cy.get('.Select-arrow-zone').click()
            cy.get("div.Select-menu-outer").as("OfficeOptions")
            cy.get("@OfficeOptions").contains(expectedOfficeOption.title).click()
        })
    ...
    it("does not display office dropdown when no offices are found", function(){
        cy.server()
        cy.route({
            method: "GET",
